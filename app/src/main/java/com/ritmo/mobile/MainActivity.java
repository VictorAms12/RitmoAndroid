...
public class MainActivity extends Activity {
    ...
    private View buildTopBar() {
        ...
        titleBox.addView(text("Ritmo", 21, TEXT, true));
        titleBox.addView(text(shortDate(Store.today()), 11, MUTED, false));
        bar.addView(titleBox, new LinearLayout.LayoutParams(0, ViewGroup.LayoutParams.WRAP_CONTENT, 1f));

        TextView version = text("v2", 10, BRAND, true);
        ...
        ImageButton theme = iconButton(darkMode ? com.ritmo.mobile.R.drawable.ic_sun : com.ritmo.mobile.R.drawable.ic_moon, darkMode ? ACCENT : BRAND);
    ...
    private View buildTasksPage() {
        ...
        for (Store.Task t : store.tasks) {
            boolean add = "all".equals(taskFilter)
        ...
        for (Store.Task t : list) body.addView(taskCard(t));
        return wrapScroll(body);
    }
    ...
    private View taskFilterBar() {
        ...
        hs.addView(row); hs.setPadding(0,0,0,dp(8));
        return hs;
    }
    ...
    private View taskCard(Store.Task t) {
        ...
        LinearLayout.LayoutParams cardP = marginBottom(dp(9));
        ...
        card.addView(check, new LinearLayout.LayoutParams(dp(36), dp(36)));

        LinearLayout center = new LinearLayout(this); center.setOrientation(LinearLayout.VERTICAL); center.setPadding(dp(11),0,dp(8),0);
        TextView title = text(t.title, 14, TEXT, true); if ("done".equals(t.status)) title.setAlpha(.5f);
        center.addView(title);
        String meta = (t.time == null || t.time.isEmpty() ? "Sem horário" : t.time) + " · " + t.category + " · " + t.minutes + " min";
        center.addView(text(meta, 11, MUTED, false));
        String extra = recurrenceLabel(t.recurrence) + (t.reminderMinutes >= 0 ? " · lembrete " + reminderLabel(t.reminderMinutes) : "");
        if (!"Sem repetição".equals(recurrenceLabel(t.recurrence)) || t.reminderMinutes >= 0) center.addView(text(extra, 10, BRAND, false));
        ...
        View dot = new View(this); dot.setBackground(rounded(priorityColor(t.priority), 99, false));
        card.addView(dot, new LinearLayout.LayoutParams(dp(9), dp(9)));
    ...
    private View weeklyPlanner() {
        ...
        head.addView(prev,new LinearLayout.LayoutParams(dp(44),dp(40))); head.addView(title,new LinearLayout.LayoutParams(0,dp(40),1f)); head.addView(next,new LinearLayout.LayoutParams(dp(44),dp(40)));
        ...
        for(int i=0;i<7;i++){
            String iso=Store.addDays(weekStart,i); int count=store.taskCountOn(iso), mins=store.plannedMinutesOn(iso), pct=store.completionPercentOn(iso);
            LinearLayout d=cardBox(); d.setOrientation(LinearLayout.VERTICAL); d.setPadding(dp(11),dp(10),dp(11),dp(10));
            if(iso.equals(selectedAgendaDate)) d.setBackground(rounded(BRAND,17,false));
            int tc=iso.equals(selectedAgendaDate)?Color.WHITE:TEXT, mc=iso.equals(selectedAgendaDate)?Color.rgb(208,239,228):MUTED;
            d.addView(text(weekdayShort(iso),10,mc,true)); d.addView(text(dayMonth(iso),15,tc,true));
            d.addView(text(count+" tarefa(s)",10,mc,false)); d.addView(text(humanMinutes(mins),10,mc,false));
            ProgressBar pb=new ProgressBar(this,null,android.R.attr.progressBarStyleHorizontal); pb.setMax(100); pb.setProgress(pct); pb.setProgressTintList(ColorStateList.valueOf(iso.equals(selectedAgendaDate)?ACCENT:BRAND)); pb.setProgressBackgroundTintList(ColorStateList.valueOf(iso.equals(selectedAgendaDate)?Color.argb(50,255,255,255):PANEL2));
            LinearLayout.LayoutParams pp=new LinearLayout.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT,dp(6)); pp.setMargins(0,dp(8),0,0); d.addView(pb,pp);
            LinearLayout.LayoutParams dpv=new LinearLayout.LayoutParams(dp(118),dp(116)); dpv.setMargins(0,0,dp(8),0); days.addView(d,dpv);
            d.setOnClickListener(v->{selectedAgendaDate=iso;syncMonthToSelected();showPage("agenda");});
        }
        hs.addView(days); card.addView(hs); return card;
    }
    ...
    private View agendaItem(Store.Task t) {
        ...
        LinearLayout event=cardBox(); event.setOrientation(LinearLayout.VERTICAL); event.setPadding(dp(14),dp(12),dp(14),dp(12));
    ...
    private View buildOrganizePage() {
        ...
        if("goals".equals(organizeTab)) body.addView(goalsSection());
    ...
    private View organizeTabs(){
        HorizontalScrollView hs=new HorizontalScrollView(this); hs.setHorizontalScrollBarEnabled(false);
        LinearLayout row=new LinearLayout(this); row.setOrientation(LinearLayout.HORIZONTAL);
        String[][] tabs={{"kanban","Kanban"},{"goals","Metas"},{"habits","Hábitos"},{"stats","Estatísticas"}};
    ...
    private View kanbanBoard(){
        ...
        for(String[] col:columns){
            ...
            int count=0; for(Store.Task t:store.tasks) if(col[0].equals(t.status)) count++;
            box.addView(text(col[1]+"  ·  "+count,11,MUTED,true));
            for(Store.Task t:store.tasks){
                if(!col[0].equals(t.status)) continue;
                LinearLayout k=cardBox(); k.setOrientation(LinearLayout.VERTICAL); k.setPadding(dp(12),dp(11),dp(12),dp(11));
                k.addView(text(t.title,13,TEXT,true)); k.addView(text(t.category+" · "+compactDate(t.date),10,MUTED,false));
                TextView hint=text("Toque para avançar · segure para opções",9,BRAND,false); hint.setPadding(0,dp(5),0,0); k.addView(hint);
                ...
                k.setOnClickListener(v->cycleTask(t)); k.setOnLongClickListener(v->{showTaskActions(t);return true;});
            }
        }
        hs.addView(board); return hs;
    }
    ...
    private View statsSection(){
        ...
        summary.addView(metric(String.valueOf(store.totalCompletedLast7()),"7 dias"),weightedMargin(0,4));
        summary.addView(metric(humanMinutes(store.totalCompletedMinutesLast7()),"Tempo concluído"),weightedMargin(4,4));
        Store.Routine best=store.bestRoutineByStreak(); summary.addView(metric(best==null?"0🔥":best.streak(Store.today())+"🔥","Melhor sequência"),weightedMargin(4,0)); wrap.addView(summary);
    ...
    private View insightsCard(){
        LinearLayout card=cardBox();card.setOrientation(LinearLayout.VERTICAL);card.setPadding(dp(15),dp(14),dp(15),dp(14));
        int pending=0,high=0,todayMin=0;for(Store.Task t:store.tasks){if(!"done".equals(t.status)){pending++;if("high".equals(t.priority))high++;}if(Store.today().equals(t.date))todayMin+=t.minutes;}
    ...
    private void showAddMenu(){
        String[] items={"Nova tarefa","Novo compromisso","Nova meta","Novo hábito"};
        new AlertDialog.Builder(this).setTitle("Adicionar").setItems(items,(d,which)->{
            ...
            else if(which==2)showGoalDialog(null);
    ...
    private void showTaskDialog(Store.Task existing, boolean eventMode, String forcedDate){
        ...
        EditText title=input("Título"); EditText description=inputMulti("Descrição / observações");
        EditText date=input("Data"); date.setFocusable(false); EditText time=input("Horário"); time.setFocusable(false);
        EditText minutes=input("Duração em minutos"); minutes.setInputType(InputType.TYPE_CLASS_NUMBER);
        ...
        form.addView(fieldLabel("TÍTULO"));form.addView(title);form.addView(fieldLabel("DESCRIÇÃO"));form.addView(description);
        form.addView(fieldLabel("DATA"));form.addView(date);form.addView(fieldLabel("HORÁRIO"));form.addView(time);
        form.addView(fieldLabel("DURAÇÃO"));form.addView(minutes);form.addView(fieldLabel("CATEGORIA"));form.addView(category);
        form.addView(fieldLabel("PRIORIDADE"));form.addView(priority);form.addView(fieldLabel("REPETIÇÃO"));form.addView(recurrence);form.addView(fieldLabel("LEMBRETE"));form.addView(reminder);
        ...
        if(editing){
            ...
            setSpinner(category,existing.category);setSpinner(priority,priorityLabel(existing.priority));setSpinner(recurrence,recurrenceLabel(existing.recurrence));setSpinner(reminder,reminderLabel(existing.reminderMinutes));
        }
        date.setOnClickListener(v->pickDate(date));time.setOnClickListener(v->pickTime(time));

        AlertDialog dlg=new AlertDialog.Builder(this).setTitle(editing?"Editar tarefa":(eventMode?"Novo compromisso":"Nova tarefa")).setView(scroll).setNegativeButton("Cancelar",null).setPositiveButton("Salvar",null).create();
        dlg.setOnShowListener(x->dlg.getButton(AlertDialog.BUTTON_POSITIVE).setOnClickListener(v->{
            ...
            if(editing){
                existing.title=ttl;existing.description=description.getText().toString().trim();existing.date=date.getText().toString();existing.time=time.getText().toString();existing.minutes=mins;existing.category=String.valueOf(category.getSelectedItem());
                existing.priority=priorityValueFromLabel(String.valueOf(priority.getSelectedItem()));existing.recurrence=recurrenceValue(String.valueOf(recurrence.getSelectedItem()));existing.reminderMinutes=reminderValue(String.valueOf(reminder.getSelectedItem()));
                store.save();ReminderScheduler.schedule(this,existing);
            }else{
                Store.Task t=new Store.Task(System.currentTimeMillis(),ttl,description.getText().toString().trim(),date.getText().toString(),time.getText().toString(),priorityValueFromLabel(String.valueOf(priority.getSelectedItem())),mins,String.valueOf(category.getSelectedItem()),"todo",recurrenceValue(String.valueOf(recurrence.getSelectedItem())),reminderValue(String.valueOf(reminder.getSelectedItem())));
                store.tasks.add(t);store.save();ReminderScheduler.schedule(this,t);
            }
    ...
    private void showTaskActions(Store.Task t){
        String[] actions={"Editar","Mover para A fazer","Mover para Em andamento","Mover para Concluído","Excluir"};
        new AlertDialog.Builder(this).setTitle(t.title).setItems(actions,(d,w)->{
            if(w==0)showTaskDialog(t,false,null);
            else if(w==1){store.setTaskStatus(t,"todo");showPage(currentPage);}
            else if(w==2){store.setTaskStatus(t,"doing");showPage(currentPage);}
            else if(w==3){store.setTaskStatus(t,"done");ReminderScheduler.cancel(this,t.id);showPage(currentPage);}
    ...
    private void showGoalDialog(Store.Goal existing){
        ...
        form.addView(fieldLabel("META"));form.addView(title);form.addView(fieldLabel("PROGRESSO (%)"));form.addView(progress);form.addView(fieldLabel("PRAZO"));form.addView(target);
        ...
        AlertDialog dlg=new AlertDialog.Builder(this).setTitle(existing==null?"Nova meta":"Editar meta").setView(form).setNegativeButton("Cancelar",null).setPositiveButton("Salvar",null).create();
        dlg.setOnShowListener(x->dlg.getButton(AlertDialog.BUTTON_POSITIVE).setOnClickListener(v->{String ttl=title.getText().toString().trim();if(ttl.isEmpty()){title.setError("Informe a meta");return;}int p=clamp(parseInt(progress.getText().toString(),0),0,100);if(existing==null)store.goals.add(new Store.Goal(System.currentTimeMillis(),ttl,p,target.getText().toString()));else{existing.title=ttl;existing.progress=p;existing.targetDate=target.getText().toString();}store.save();dlg.dismiss();organizeTab="goals";showPage("organize");}));dlg.show();
    }

    private void showRoutineDialog(Store.Routine existing){
        LinearLayout form=dialogForm();EditText title=input("Nome do hábito");EditText detail=input("Descrição curta");EditText minutes=input("Duração em minutos");minutes.setInputType(InputType.TYPE_CLASS_NUMBER);Spinner frequency=spinner(new String[]{"Todos os dias","Seg a Sex","Semanal"});
        form.addView(fieldLabel("HÁBITO"));form.addView(title);form.addView(fieldLabel("DESCRIÇÃO"));form.addView(detail);form.addView(fieldLabel("DURAÇÃO"));form.addView(minutes);form.addView(fieldLabel("FREQUÊNCIA"));form.addView(frequency);
        ...
        AlertDialog dlg=new AlertDialog.Builder(this).setTitle(existing==null?"Novo hábito":"Editar hábito").setView(form).setNegativeButton("Cancelar",null).setPositiveButton("Salvar",null).create();
        dlg.setOnShowListener(x->dlg.getButton(AlertDialog.BUTTON_POSITIVE).setOnClickListener(v->{String ttl=title.getText().toString().trim();if(ttl.isEmpty()){title.setError("Informe o hábito");return;}String f=frequencyValue(String.valueOf(frequency.getSelectedItem()));int mins=parseInt(minutes.getText().toString(),15);if(existing==null)store.routines.add(new Store.Routine(System.currentTimeMillis(),ttl,detail.getText().toString().trim(),f,mins,Store.today()));else{existing.title=ttl;existing.detail=detail.getText().toString().trim();existing.frequency=f;existing.minutes=mins;}store.save();dlg.dismiss();organizeTab="habits";showPage("organize");}));dlg.show();
    }
    ...
    private LinearLayout dialogForm(){LinearLayout form=new LinearLayout(this);form.setOrientation(LinearLayout.VERTICAL);form.setPadding(dp(20),dp(6),dp(20),dp(8));return form;}
    private TextView fieldLabel(String label){TextView t=text(label,10,MUTED,true);t.setPadding(dp(3),dp(4),0,dp(5));return t;}

    private EditText input(String hint){EditText e=new EditText(this);e.setHint(hint);e.setTextSize(14);e.setSingleLine(true);e.setTextColor(TEXT);e.setHintTextColor(MUTED);e.setPadding(dp(12),dp(9),dp(12),dp(9));e.setBackground(rounded(PANEL2,12,false));LinearLayout.LayoutParams p=new LinearLayout.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT,dp(50));p.setMargins(0,0,0,dp(9));e.setLayoutParams(p);return e;}
    private EditText inputMulti(String hint){EditText e=new EditText(this);e.setHint(hint);e.setTextSize(14);e.setTextColor(TEXT);e.setHintTextColor(MUTED);e.setGravity(Gravity.TOP);e.setMinLines(2);e.setMaxLines(4);e.setPadding(dp(12),dp(10),dp(12),dp(10));e.setBackground(rounded(PANEL2,12,false));LinearLayout.LayoutParams p=new LinearLayout.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT,dp(82));p.setMargins(0,0,0,dp(9));e.setLayoutParams(p);return e;}
    private Spinner spinner(String[] items){Spinner s=new Spinner(this);ArrayAdapter<String> a=new ArrayAdapter<>(this,android.R.layout.simple_spinner_dropdown_item,items);s.setAdapter(a);s.setBackground(rounded(PANEL2,12,false));LinearLayout.LayoutParams p=new LinearLayout.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT,dp(50));p.setMargins(0,0,0,dp(9));s.setLayoutParams(p);return s;}
    ...
    private String priorityLabel(String p){if("high".equals(p))return "Alta";if("medium".equals(p))return "Média";return "Baixa";}
    private String priorityValueFromLabel(String p){if("Alta".equals(p))return "high";if("Média".equals(p))return "medium";return "low";}
